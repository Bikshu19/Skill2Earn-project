...
const createUserDetails = async (req, res) => {
  try {
    ...
    const { location, address, pincode, mobileNumber, whatsappNumber } = req.body;
    ...
    const existing = await User.findById(decoded.id).select('location address pincode mobileNumber whatsappNumber');
    if (existing && existing.location && existing.address && existing.mobileNumber && existing.whatsappNumber) {
      return res.status(400).json({ message: 'Details already exist. Use update endpoint instead.' });
    }
    ...
    res.status(201).json({ message: 'Details saved successfully', userDetails: updatedUser });
  } catch (error) {
    console.error('Create user details error:', error);
    res.status(500).json({ message: error.message || 'Server error while saving details' });
  }
};
...
const updateUserDetails = async (req, res) => {
  try {
    ...
    if (!updatedUser) {
      return res.status(404).json({ message: 'User details not found. Use create instead.' });
    }

    res.status(200).json({ message: 'Details updated successfully', userDetails: updatedUser });
  } catch (error) {
...
const addSkillToUser = async (req, res) => {
  try {
    ...
    if (!category || !subcategory || !description)
      return res.status(400).json({ message: 'Category, subcategory, and description are required' });
    ...
    return res.status(201).json({ message: 'Skill added', works: updatedUser.works });
  } catch (error) {
...
const sendRequest = async (req, res) => {
  try {
    ...
    if (owner.gender === "female" && requester.gender === "female") {
      ...
      work.requests.push({ requesterId, status: "pending", requestedAt: new Date() });
      await user.save();
      ...
      return res.status(200).json({ message: "Email notification sent instead of direct request." });
    }
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
const updateRequestStatus = async (req, res) => {
  try {
    ...
    const work = user.works.id(workId);
    const request = work.requests.find(r => r.requesterId.equals(requesterId));
  ...
  try {
    ...
    const detailedRequests = await Promise.all(
      work.requests
        ...
        .map(async r => {
          const requester = await User.findById(r.requesterId, "username email gender");
...
const getUserContactInfo = async (req, res) => {
  try {
    ...
    const user = await User.findById(userId, 'username email gender mobileNumber whatsappNumber');
    if (!user) {
      return res.status(404).json({ message: 'User not found.' });
    }
...
module.exports = { registerUser,loginUser,getUserProfile ,updateUserProfile,createUserDetails,updateUserDetails,addSkillToUser,getUsersWithPendingWorks,deleteUserById,publishWork,deleteUserId,deleteWork,getUsersWithPublicWorks,sendRequest,
  updateRequestStatus,
  getRequestsForWork,
  manualSendEmail  };
